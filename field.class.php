...
class profile_field_radio extends profile_field_base {
     ...
     * Constructor method.
     *
     * Pulls out the options for the radio from the database and sets the the corresponding key for the data if it exists.
     *
     * @param int $fieldid
     * @param int $userid
    ...
    public function edit_field_add($mform) {
        $mform->addElement('static', 'radiolabel', format_string($this->field->name));
        foreach ($this->options as $option) {
            $mform->addElement('radio', $this->inputname, null, $option, $option);
        }
        if ($this->field->required) {
            $mform->addRule($this->inputname, null, 'required', null, 'client');
        }
    }
     ...
     *
     * @param string $value one of the values in radio options.
     * @return int options key for the radio
     */
