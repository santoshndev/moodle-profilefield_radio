...
/**
 * Base class for unit tests for profilefield_radio.
 *
 * @package    profilefield_radio
 * @copyright  2025 Santosh N. <santosh.nag2217@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace profilefield_radio\privacy;

use core_privacy\tests\provider_testcase;
use profilefield_radio\privacy\provider;
use core_privacy\local\request\approved_userlist;

/**
 * Unit tests for user\profile\field\radio\classes\privacy\provider.php
 *
 * @copyright  2025 Santosh N. <santosh.nag2217@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
final class provider_test extends provider_testcase {
    ...
    public function test_get_contexts_for_userid(): void {
        ...
        $profilefieldid = $this->add_profile_field($categoryid, 'radio');
        // Create a user.
    ...
    public function test_export_user_data(): void {
        // Create profile category.
        $categoryid = $this->add_profile_category();
        // Create radio profile field.
        $radioprofilefieldid = $this->add_profile_field($categoryid, 'radio');
        // Create checkbox profile field.
        $checkboxprofilefieldid = $this->add_profile_field($categoryid, 'checkbox');
        // Create a user.
        $user = $this->getDataGenerator()->create_user();
        $context = \context_user::instance($user->id);
        // Add radio user info data.
        $this->add_user_info_data($user->id, $radioprofilefieldid, 'test radio');
        // Add checkbox user info data.
        $this->add_user_info_data($user->id, $checkboxprofilefieldid, 'test data');
        $writer = \core_privacy\local\request\writer::with_context($context);
        $this->assertFalse($writer->has_any_data());
        $this->export_context_data_for_user($user->id, $context, 'profilefield_radio');
        $data = $writer->get_data([get_string('pluginname', 'profilefield_radio')]);
        $this->assertCount(3, (array) $data);
        $this->assertEquals('Test field', $data->name);
        $this->assertEquals('This is a test.', $data->description);
        $this->assertEquals('test radio', $data->data);
    }
    ...
    public function test_delete_data_for_all_users_in_context(): void {
        ...
        $this->assertNotEquals('test radio', reset($userinfodata)->data);
    }
